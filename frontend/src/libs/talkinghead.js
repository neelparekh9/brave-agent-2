  ...
  lipsyncGetProcessor(lang, path = "./") {
    if (!this.lipsync.hasOwnProperty(lang)) {
      if (lang === 'en') {
        ...
        console.log("Successfully loaded LipsyncEn explicitly");
      } else {
        const moduleName = path + "lipsync-" + lang.toLowerCase() + ".js";
        const className = "Lipsync" + lang.charAt(0).toUpperCase() + lang.slice(1);

        console.log(`Attempting to dynamically import via path: ${moduleName}`);
        ...
        import(moduleName)
          ...
          .catch((error) => {
            console.error(`Failed to dynamically load module: ${moduleName}`, error);
          });
      }
    }
  }
